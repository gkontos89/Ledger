...
public class TcpServer 
{
	...
	public boolean sendMessage(MarshmallowMessage msg, User user) throws IOException
	{
		boolean itWorked = true;
		if(user != null && user.getMyConnectionThread() != null)
			itWorked = user.getMyConnectionThread().sendData(msg.getAsByteArray());
		else
		{
			// Send to everyone if we dont know who to send too 
			for(TcpConnectionHandlerThread client : myThreadHandlers)
			{
